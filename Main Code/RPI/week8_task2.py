...
class MultiProcessCommunication:
    def __init__(self):
     ...
     #    self.IMAGE_LIST = self.manager.list()
        ...
        labelA = Value('i', 0) #[IMAGE,CONF]
        confidence = Value('d', 0.0)
        self.action = [0,0] #action[0] stores left or right on first obs, action[1] stores the count of obs
        lock = Lock()
        moveNext = Value('i', 1) # for STM to tell RPI it is ready for next command.
        self.read_stm_process = Process(target=self._read_stm, args=(moveNext,lock,labelA))
        ...
        self.sender = None
        # self.image_process = Process(target=self._process_pic, args=(labelA, confidence, lock))
    ...
    def start(self):
        try:
            ...
            print(Fore.LIGHTGREEN_EX + str(startComms_dt) + '| [MultiProcess] Communications started. Reading from STM & Android')

          #   # Start the Image Rec process
            # self.image_process.start()
          #   startImage_dt = datetime.now().strftime('%d-%b-%Y %H:%M%S')
            print(Fore.LIGHTGREEN_EX + '| [MultiProcess-IR] Image Processing Mode is ON')
    ...
    def _read_stm(self, moveNext, lock, labelA):
        while True:
            try:
                ...
                if len(raw_message_list) != 0:
                    ...
                    if message_list[0] == 'AND':
                        ...
                        print('returning image id...')
                        # labelB = ''
                        # with lock:
                        #     if labelA.value == 38:
                        #         labelB = 'r'
                        #     elif labelA.value == 39:
                        #         labelB = 'l'
                        # if self.action[1] == 0:
                        #     print("LABELDIR: ", labelB)
                        #     message = 't' + labelB + "n"
                        #     self.action[0] = labelB
                        #     self.action[1] = 1
                        ...
                        label = imgRec() 
                        #In the format of OBSTACLE_NUM:IMAGE_ID
                        label = label.split(";", 1)
                        movement = ''
                        if label[0] == '38':
                            movement = 'r'
                        ...
                        if self.action[1] == 0:
                            print("LABELDIR: ", movement)
                            message = 't' + movement + "n"
                            self.action[0] = movement
                            self.action[1] = 1
                        ...
                        #send image id back to STM
                        
                        print(Fore.LIGHTCYAN_EX + 'RPI > STM , %s' % message)
                        for char in message:
                            self.message_queue.put_nowait(self._format_for("STM", char.encode(LOCALE)))

                        # self.message_queue.put_nowait(self._format_for("STM", message.encode(LOCALE)))
    ...
    def _read_algorithm(self):
        ...
        while True:
            try:
                ...
                print("raw_message is: ", raw_message)
                ...
                raw_message_list = raw_message.decode().splitlines()
                print("Raw Message List is: ",raw_message_list)
                ...
                for pre_message_list in raw_message_list:
                    if len(pre_message_list) != 0:
                        ...
                        if message_list[0] == 'RPI':
                            ...
                            elif "PLOT" in message_list[1]:
                                print(Fore.LIGHTCYAN_EX + 'Algo > AND , %s' % (message_list[1]))
                                #self.to_android_message_queue.put_nowait(plotValue.encode(LOCALE))
                                self.message_queue.put_nowait(self._format_for("AND", message_list[1].encode(LOCALE)))

                        elif message_list[0] == 'AND':
                            plotValue = "PLOT," + message_list[1]
                            print(Fore.LIGHTCYAN_EX + 'Algo > %s , %s' % (str(message_list[0]), plotValue))
                            #self.to_android_message_queue.put_nowait(plotValue.encode(LOCALE))
                            self.message_queue.put_nowait(self._format_for(message_list[0], plotValue.encode(LOCALE)))
                ...
                for pre_message_list in raw_message_list:
                    if len(pre_message_list) != 0:
                        ...
                        if message_list[0] == 'ALG':
                            ...
                            # Send to STM for movements
                            if message_list[1] == 'sp':
                                print(Fore.LIGHTCYAN_EX + 'AND > %s , %s' % (str(message_list[0]), str(message_list[1])))
                                sent = 'un'
                                print(Fore.LIGHTCYAN_EX + 'AND > %s , %s' % (str(message_list[0]), sent))
                                for char in sent:
                                    self.message_queue.put_nowait(self._format_for("STM", char.encode(LOCALE)))
                                # self.message_queue.put_nowait(self._format_for(message_list[0], sent.encode(LOCALE)))
    ...
    def _process_pic(self, labelA, confidence, lock): # %terminate%
        ...
        while True:
            ...
            #             print('ID sent to Android')
                ...
                label_message = imgRec() #format of [IMAGE_ID,CONF]
                label = label_message.split(';', 1)
                label[1] = float(label[1])
                print("IMAGE LABEL: ",label)
                with lock:
                    if labelA.value == 0:
                        labelA.value = int(label[0])
                        confidence.value = label[1]              
                    elif label[0] != -1:
                        labelA.value = int(label[0])
                        confidence.value = label[1]
                time.sleep(0.1)
