...
# from rpi.socket_v2.server import *
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # img_count = 0
            # if image_label == '38':
            #     while os.path.exists("imgr%s.jpg" % img_count):
            #         img_count += 1
            #     cv2.imwrite(f'imgr{img_count}.jpg', im0)

            # img_count = 0
            # if image_label == '39':
            #     while os.path.exists("imgl%s.jpg" % img_count):
            #         img_count += 1
            #     cv2.imwrite(f'imgl{img_count}.jpg', im0)
...
def handle_client(conn, addr):
    try:
        ...
        while True:
            ...
            f = open(f'rpi//images/rec1.jpg', 'wb') # tbc
    ...
    while True:
        try:
            ...
            while True:
                ...
                image_label, image_conf = run(**vars(opt))
                #sent_label = "ALG:" + image_label

                if image_label != '-1' and image_label != 'bullseye':
                    print("The arrow is: ", IMG_MAP[image_label])
                ...
                sent = str(image_label) + ";" + str(image_conf)
                # sent = [IMG_MAP[image_label],image_conf]

                conn.send(f'{sent}'.encode())
                print(sent," is sent successfully")
